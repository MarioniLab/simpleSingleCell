...
For endogenous genes, normalization is performed using the `simpleSumFactors()` function as `r Biocpkg("simpleSingleCell", "work-1-reads.html#normalization-of-cell-specific-biases", label="previously described")` with some modifications:
...
- We set `approximate=TRUE` to speed up the dimensionality reduction preceding the nearest-neighbour search.
