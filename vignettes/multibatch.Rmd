...
This has already been processed into a `SingleCellExperiment` by the `r Biocpkg("scRNAseq")` package.

```{r}
library(scRNAseq)
sce.emtab <- SegerstolpePancreasData()
sce.emtab
```

We translate the gene symbols into Ensembl gene identifiers via the `r Biocpkg("org.Hs.eg.db")` package.

```{r}
library(org.Hs.eg.db)
symbols <- rowData(sce.emtab)$symbol
ens.id <- mapIds(org.Hs.eg.db, keys=symbols,
    keytype="SYMBOL", column="ENSEMBL")
ens.id <- ifelse(is.na(ens.id), symbols, ens.id)

keep <- !duplicated(ens.id)
sce.emtab <- sce.emtab[keep,]
rownames(sce.emtab) <- ens.id[keep]
head(rownames(sce.emtab))
```

We simplify the names of some of the relevant column metadata fields for ease of access.

```{r}
emtab.meta <- colData(sce.emtab)[,c("cell type", 
    "individual", "single cell well quality")]
colnames(emtab.meta) <- c("CellType", "Donor", "Quality")
emtab.meta$Study <- "E-MTAB-5061"
colData(sce.emtab) <- emtab.meta
colData(sce.emtab)
```

Some editing of the cell type labels is also necessary for consistency with the annotation used in GSE86469.

```{r}
sce.emtab$CellType <- gsub(" cell", "", sce.emtab$CellType)
sce.emtab$CellType <- paste0(
    toupper(substr(sce.emtab$CellType, 1, 1)),
    substring(sce.emtab$CellType, 2))
table(sce.emtab$CellType)
```
