...
date: "`r Sys.Date()`"
vignette: >
  %\VignetteIndexEntry{12. Advanced batch correction}
...
Some editing of the cell type labels is necessary for consistency with GSE86473.
...
universe <- Reduce(intersect, list(rownames(dec.gse81076), 
    rownames(dec.gse85241), rownames(dec.gse86473), 
    rownames(dec.emtab)))
...
nout <- multiBatchNorm(sce.gse81076[universe,], sce.gse85241[universe,],
    sce.gse86473[universe,], sce.emtab[universe,])
sce.gse81076 <- nout[[1]]
sce.gse85241 <- nout[[2]]
sce.gse86473 <- nout[[3]]
sce.emtab <- nout[[4]]
...
mean.bio <- (dec.gse81076[universe,"bio"] + dec.gse85241[universe,"bio"]
    + dec.gse86473[universe,"bio"] + dec.emtab[universe,"bio"])/4
...
sce.gse86473 <- sce.gse86473[chosen,]
...
- The `IrlbaParam(deferred=TRUE)` parameter instructs `multiBatchPCA()` to perform a fast approximate PCA with methods from the `r CRANpkg("irlba")` package.
...
## Merging the SMART-seq2 data

The @segerstolpe2016singlecell study contains strong donor effects that interfere with correction.
Several cell types exhibit strong per-donor effects such that the multiple clusters cannot be fully merged with the corresponding single cluster in GSE86473 (Figure \@ref(fig:smart-raw)).

```{r smart-raw, fig.wide=TRUE, fig.asp=0.5, fig.cap="t-SNE plots of the merged SMART-seq2 data sets. Each point is a cell coloured by the batch of origin (left) or the donor of origin in E-MTAB-5061 (right)."}
direct.smart <- fastMNN(pcs$gse86473, pcs$emtab, k=20, pc.input=TRUE)

set.seed(2000)
tsne.out <- Rtsne::Rtsne(direct.smart$corrected, perplexity=30, pca=FALSE)
df <- data.frame(x=tsne.out$Y[,1], y=tsne.out$Y[,2], 
    batch=rep(c("GSE86473", "E-MTAB-5061"), 
        c(ncol(sce.gse86473), ncol(sce.emtab))),
    donor=c(rep("unknown", ncol(sce.gse86473)), sce.emtab$Donor))
...
It is then straightforward to merge these corrected expression values with the data from GSE86473.
This removes differences between studies and represents the second level of the merge hierarchy.

```{r}
mnn.smart <- fastMNN(pcs$gse86473, fixed.emtab, k=20, pc.input=TRUE)
```
...
```{r smart-fixed, fig.wide=TRUE, fig.asp=0.5, fig.cap="t-SNE plots of the merged SMART-seq2 data sets. Each point is a cell coloured by the batch of origin (left) or the donor of origin in E-MTAB-5061 (right)."}
set.seed(2000)
...
Figure \ref(fig:cel-merge) demonstrates that these two data sets are successfully merged.
...
df <- data.frame(x=tsne.out$Y[,1], y=tsne.out$Y[,2], 
    batch=rep(c("GSE81076", "GSE85241", "GSE86473", "E-MTAB-5061"), 
        c(ncol(sce.gse81076), ncol(sce.gse85241),
            ncol(sce.gse86473), ncol(sce.emtab))),
    type=c(rep("unknown", ncol(sce.gse81076)+ncol(sce.gse85241)), 
        sce.gse86473$CellType, sce.emtab$CellType),
