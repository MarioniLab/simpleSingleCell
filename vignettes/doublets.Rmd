...
It is difficult to define a general fixed threshold above which libraries are to be considered doublets.
Moreover, not all real doublets will have high doublet scores due to the inevitable differences between real data and the simulation scheme in `doubletCells()`.
Simply removing all cells with high doublet scores will not be sufficient to eliminate doublets from the data set.
We recommend using these scores in the context of cluster annotation, where entire clusters with higher-than-average doublet scores are considered suspect.
