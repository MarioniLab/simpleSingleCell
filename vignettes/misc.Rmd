...
For example, `computeSumFactors()` will apply a somewhat stringent filter based on the average count, while `trendVar()` will apply a relatively relaxed filter based on the average log-expression.
...
The example below demonstrates how we _could_ remove genes with average counts less than 1 in the `r simpleSingleCell:::.link("reads", NULL, "416B dataset")`.
...
ave.counts <- calculateAverage(sce.416b)
demo.keep <- ave.counts >= 1
filtered.sce.416b <- sce.416b[demo.keep,]
summary(demo.keep)
```
