...
Ranking based on $p$-value tends to prioritize HVGs that are more likely to be true positives but, at the same time, less likely to be interesting.
...
## HVGs and the bias-variance trade-off

A common procedure in many scRNA-seq data analyses is to select a subset of interesting genes for further analysis.
The definition of "interesting" is often based on a per-gene measure of variation such that HVGs are prioritized.
For example, `denoisePCA()` does so when it only performs the principal components analysis on genes with positive biological components.
More aggressive strategies involve taking some top set of genes, 
e.g., the top 1000-5000 genes with the largest biological components or the smallest $p$-values.

The choice between these strategies involves making a decision about the acceptable bias-variance trade-off in the analysis.
If we only use a small set of HVGs, we risk discarding genuine biological signal and our bias will likely be high.
Conversely, if we include too many genes, we risk obscuring the signal with unnecessary noise.
It is difficult to determine the optimal trade-off for any given application,
as noise in one context may be useful signal in another.
For example, heterogeneity in T cell activation responses is an interesting phenomena [@richard2018tcell]
but may be simply noise in studies that only care about distinguishing the major immunophenotypes.

The position taken by most of our workflows is to err on the side of reducing bias at the cost of increasing variance.
This is done by retaining as many genes as possible for further analysis and only removing what is strictly necessary.
The motivation here is to provide a conservative baseline analysis that avoids loss of structure in pathological scenarios.
...
Relaxed selection of genes avoids these issues to improve reliability at the cost of suboptimal signal-to-noise.

That said, once a baseline is acquired, users can easily explore various compromises between bias and variance.
This is achieved in most `r Biocpkg("scran")` functions through the `subset.row=` argument (or equivalent in other packages),
which allows users to easily test different gene subsets in various procedures.
The example below uses the top 500 genes with the largest biological components for dimensionality reduction via `denoisePCA()`.

```{r}
top.hvgs <- head(rownames(hvg.out), 500)
sce.hsc <- denoisePCA(sce.hsc, technical=var.fit$trend,
    subset.row=top.hvgs, max.rank=50)
ncol(reducedDim(sce.hsc, "PCA"))
```
...
In such cases, a useful procedure is to repeat the entire analysis on each cluster of cells identified in the initial analysis.
To do so, users can simply subset the `SingleCellExperiment` object and re-apply functions for variance modelling, dimensionality reduction, clustering, etc. to the subset.
This allows characterization of further heterogeneity within each cluster that was masked in the overall analysis.
Of course, this strategy is much more labour intensive -
in addition to the obvious additional work, more care is required to deal with the consequences of overclustering at each step.
    - Overclustering in the initial analysis splits subpopulations in a manner that does not reflect the major factors of variation within each subpopulation (as the HVGs capturing that subpopulation's internal variation are not used).
    ...
    - Overclustering reduces the number of cells in each of the repeated analyses,
    ...
    - The alternative is to avoid overclustering by requiring strong separation at each step,
    which increases the number of steps required to achieve a particular resolution.

## Further comments

We save the HSC dataset to file for later use.

```{r}
saveRDS(sce.hsc, file="hsc_data.rds")
```

Note that there are many other strategies for defining HVGs, based on a variety of metrics:
