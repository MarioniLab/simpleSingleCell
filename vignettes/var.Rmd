...
That said, once a baseline is acquired, users can easily explore various compromises between bias and variance in most functions through the `subset.row=` argument (or equivalent).
This allows users to test different gene subsets without having to keep track of changes to the input `SingleCellExperiment`.
...
- If too few HVGs are selected, the analysis may fail to resolve heterogeneity that is not the most dominant factor in the data.
In such cases, a useful sanity-checking procedure is to repeat the entire analysis (including HVG detection)
on each subpopulation.
This allows characterization of further heterogeneity within each subpopulation that was masked in the overall analysis.
Of course, this is not without its own risks - repeated application of this strategy can result in the definition of highly resolved "subpopulations" with differences that are so small as to be meaningless in the wider context of the entire population.
