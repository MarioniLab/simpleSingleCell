...
The code below contains a no-frills analysis of the @zeisel2015brain data set, culminating in Figure \@ref(fig:quickstart).
This is only provided as a demonstration of the overall structure of a basic scRNA-seq analysis -
for each step, more sophisticated procedures will be presented in the following workflows.
(While this simple analysis is unlikely to do too poorly if applied directly to a user's data set,
it is still helpful to read the workflows to gain an understanding of the potential problems at each step and their solutions.)

```{r quickstart, fig.cap="A t-SNE plot of the Zeisel brain dataset, where each cell is a point coloured by its cluster assignment."}
# Preparing data:
library(scRNAseq)
sce <- ZeiselBrainData()
