...
```{r}
islam.fname <- "GSE29087_L139_expression_tab.txt.gz"
if (!file.exists(islam.fname)) {
    download.file("https://www.ncbi.nlm.nih.gov/geo/download/?acc=GSE29087&format=file&file=GSE29087%5FL139%5Fexpression%5Ftab%2Etxt%2Egz",
        islam.fname, mode="wb")
}
```
...
```{r}
wilson.fname <- "GSE61533_HTSEQ_count_results.xls.gz"
if (!file.exists(wilson.fname)) {
    download.file("https://www.ncbi.nlm.nih.gov/geo/download/?acc=GSE61533&format=file&file=GSE61533%5FHTSEQ%5Fcount%5Fresults%2Exls%2Egz", 
        wilson.fname, mode="wb")
}
library(R.utils)
gunzip(wilson.fname, remove=FALSE, overwrite=TRUE)
```
...
```{r, echo=FALSE, results='hide'}
all.urls <- "http://www.nature.com/nbt/journal/v33/n2/extref/nbt.3102-S7.xlsx"
mahata.fname <- basename(all.urls)
if (!file.exists(mahata.fname)) { 
    download.file(all.urls, mahata.fname, mode="wb")
}
```
