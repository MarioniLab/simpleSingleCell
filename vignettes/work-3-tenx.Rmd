...
We apply the pooling method to compute size factors for all cells [@lun2016pooling].
Refer to the `r Biocpkg("simpleSingleCell", "work-2-umis.html#normalization-of-cell-specific-biases", label="previous workflow")` for an explanation of the arguments.
...
In such cases, we can set `block=` in `simpleSumFactors()` to cluster cells within each batch or run.
...
- `simpleSumFactors()` can process batches in parallel via the `r Biocpkg("BiocParallel")` framework.
In this manner, size factors for large datasets can be computed in a scalable manner.
Users can specify their desired parallelization strategy via the `BPPARAM` argument.
...
```{r heatmap, fig.wide=TRUE, fig.cap="Heatmap of mean-centred and normalized log-expression values for the top set of markers for cluster 7 in the PBMC dataset. Column colours represent the cluster to which each cell is assigned, as indicated by the legend."}
