...
date: "`r Sys.Date()`"
vignette: >
  %\VignetteIndexEntry{Episode 4: analyzing droplet-based scRNA-seq data}
...
Ideally, we would remove cells with low library sizes or total number of expressed features as described [previously](https://bioconductor.org/packages/release/workflows/vignettes/simpleSingleCell/inst/doc/work-1-reads.html#quality-control-on-the-cells).
...
We recommend performing the analysis first with a stringent QC filter, and then relaxing it based on further diagnostics (see [here](http://bioconductor.org/packages/devel/workflows/vignettes/simpleSingleCell/inst/doc/work-4-misc.html#checking-for-discarded-cell-types) for an example).
...
We perform some pre-clustering to break up obvious clusters, as described [previously](http://bioconductor.org/packages/devel/workflows/vignettes/simpleSingleCell/inst/doc/work-2-umis.html#normalization-of-cell-specific-biases).
...
- Larger droplet-based datasets will often be processed in separate batches or runs.
...
(see comments [here](http://bioconductor.org/packages/devel/workflows/vignettes/simpleSingleCell/inst/doc/work-2-umis.html#6_normalization_of_cell-specific_biases) for a discussion of the considerations involved in pre-clustering for normalization).
Even in the absence of any known batch structure, we can improve speed by setting an arbitrary factor, e.g., using `block=rep(seq_len(10), length.out=ncol(sce))` to split the cells into ten "batches" of roughly equal size.
...
(see [here](http://bioconductor.org/packages/devel/workflows/vignettes/simpleSingleCell/inst/doc/xtra-3-var.html#32_when_spike-ins_are_unavailable) for details).
...
(see the [corresponding analysis](http://bioconductor.org/packages/devel/workflows/vignettes/simpleSingleCell/inst/doc/work-2-umis.html#7_modelling_and_removing_technical_noise) of the @zeisel2015brain data set).
