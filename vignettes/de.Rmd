...
simpleSingleCell:::.compile("multibatch") # pancreas
```
...
The effect size is reported as the "area under the curve",
i.e., the probability of randomly sampling one observation in one cluster that is greater than a random observation in another cluster.
...
Another approach is to use binomial tests to check for differences in the proportion of expressing cells between clusters.
(A cell is considered to express a gene simply if it has a non-zero count for that gene.)
The effect size is reported as the log-fold change in this proportion between each pair of clusters.
We demonstrate below using the 416B data, focusing on genes that are upregulated in each cluster with `direction="up"`.

```{r}
b.pairs <- pairwiseBinom(sce.416b, sce.416b$cluster, block=sce.416b$Plate,
    direction="up")
b.out <- combineMarkers(b.pairs$statistics, b.pairs$pairs, effect="logFC")
head(b.out[["1"]]) # top DEGs for cluster 1 against the others.
```

Large log-odds ratios indicate that the gene is more frequently expressed in one cluster compared to the other.
Again, we see strong differences in the expressed proportion between cluster 1 and the others (Figure \@ref(fig:viol-de-binom))
for the top set of genes in `b.out`.

```{r viol-de-binom, fig.cap="Distribution of log-normalized expression values for the top 10 DE genes involving cluster 1 with the binomial test, stratified by cluster assignment and coloured by the plate of origin for each cell."}
plotExpression(sce.416b, x="cluster", colour_by="Plate",
    features=head(rownames(b.out[[1]])))
```

Binomial tests offer a much more stringent definition of marker genes,
where the top-ranking genes are those that are expressed frequently in one cluster and not expressed at all in another.
This can prioritize useful genes more effectively but can also discard good candidate markers.
For example, _GCG_ is a clear marker for pancreatic alpha cells but is expressed in almost every other cell of the @lawlor2017singlecell `r simpleSingleCell:::.link("multibatch", "loading in the data", "pancreas data")` (Figure \@ref(fig:viol-gcg-lawlor)).
...
sce.gse86469 <- readRDS("gse86469_sce.rds")
plotExpression(sce.gse86469, x="CellType", features="ENSG00000115263")
```

Another aspect of this test is that it will not respond to scaling normalization.
