...
Firstly, we convert each summed pseudo-bulk count into an average, weighted by the number of cells used to compute it.
Higher weights reflect the greater precision of average counts derived from many cells.

```{r}
stopifnot(identical(colnames(summed), names(num)))
averaged <- sweep(summed, 2, num, "/")

library(edgeR)
y <- DGEList(averaged)
y$weights <- matrix(as.integer(num), nrow(y), ncol(y), byrow=TRUE)
```

We ignore spike-in transcripts and low-abundance genes, and we compute normalization factors^[Not the same as size factors!] using `calcNormFactors()`.
