...
We download both the count tables (in the "processed files") as well as the metadata file using the `r Biocpkg("BiocFileCache")` framework.
This saves the files to a local cache and avoids re-downloading them if they are already present.
...
We demonstrate this approach later with a larger dataset in the next `r Biocpkg("simpleSingleCell", vignette="work-2-umis.html#6_normalization_of_cell-specific_biases", label="workflow")`.
...
An alternative approach is to use linear models via the `design=` argument - the differences between the two are discussed `r Biocpkg("simpleSingleCell", vignette="work-5-mnn.html#using-the-corrected-values-in-downstream-analyses", label="here")`.
