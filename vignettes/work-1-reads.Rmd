...
We can explore the effect of known biological factors on the QC metrics using the `plotColData()` function.
...
multiplot(
    plotColData(sce, y="total_counts", x="Oncogene"),
    plotColData(sce, y="total_features", x="Oncogene"),
    plotColData(sce, y="pct_counts_ERCC", x="Oncogene"),
    plotColData(sce, y="pct_counts_Mt", x="Oncogene"),
    cols=2)
```
...
- An alternative strategy for choosing the number of PCs is to use the `parallelPCA()` function, which performs Horn's parallel analysis [@horn1965rationale].
This involves permuting the matrix and repeating the PCA to determine the variance explained by each PC under a random null.
In this manner, "p-values" can be obtained for each PC [@buja1992remarks].
PCs are discarded if they do not explain significantly more variance than expected under the null.
Parallel analysis tends to yield better estimates of the rank than `denoisePCA()`, but is obviously much slower due to the need for multiple permutations.
It also relies on random permutations, so setting the random seed is necessary for reproducible results.

    ```{r}
    set.seed(1000)
    npcs <- parallelPCA(sce, assay.type="corrected", value="n")
    as.integer(npcs)
    ```
...
```{r pcaplot416b-onco, fig.cap="Pairwise PCA plots of the first three PCs in the 416B data set, constructed from normalized log-expression values of genes with positive biological components. Each point represents a cell, coloured according to oncogene induction status.", fig.width=9}
...
```{r pcaplot416b-batch, fig.cap="Pairwise PCA plots of the first three PCs in the 416B data set, constructed from normalized log-expression values of genes with positive biological components. Each point represents a cell, coloured according to the plate of origin.", fig.width=9}
plotReducedDim(sce, use_dimred="PCA", ncomponents=3, 
