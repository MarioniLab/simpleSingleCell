...
- See `r Biocpkg("simpleSingleCell", vignette="xtra-1-qc.html#assumptions-of-outlier-identification", label="this section")` for a more detailed discussion of the assumptions underlying the outlier-based detection of low-quality cells.
...
```{r normplot416b, fig.cap="Size factors from pooling, plotted against library sizes for all cells in the 416B dataset. Axes are shown on a log-scale. Wild-type cells are shown in black and oncogene-induced cells are shown in red."}
...
- Older versions of this workflow used the `computeSumFactors()` function, which implements the method described by @lun2016pooling exactly.
`simpleSumFactors()` provides some advantages over the original method in that it does not require a pre-clustering step; will not yield negative size factors; 
and is more accurate, provided there are at least `k` cells (default 20) in each local neighbourhood around each cell.
...
See `r Biocpkg("simpleSingleCell", vignette="xtra-3-var.html#details-of-trend-fitting-parameters", label="here")` for more details on how to refine the fit.
- In the absence of spike-ins, users can set `use.spikes=FALSE` to fit a trend to the variances of the endogenous genes (see `r Biocpkg("simpleSingleCell", vignette="xtra-3-var.html#when-spike-ins-are-unavailable", label="here")`).
Alternatively, we can create a trend based on the assumption of Poisson technical noise, as described `r Biocpkg("simpleSingleCell", vignette="work-3-tenx.html#modelling-the-mean-variance-trend", label="here")`.
...
The aim is to test for DE in each cluster compared to the others while blocking on uninteresting factors such as the plate of origin (see `r Biocpkg("simpleSingleCell", "xtra-3b-de.html#blocking-on-uninteresting-factors-of-variation", label="here")` for details).
...
This is because the clusters have been empirically identified from the data, see `r Biocpkg("simpleSingleCell", vignette="xtra-3b-de.html#misinterpretation-of-de-p-values")`.
