...
sce$DoubletDensity <- dbl.dens
plotTSNE(sce, colour_by="DoubletDensity")
```
...
plotColData(sce, x="Cluster", y="DoubletDensity", colour_by="Cluster")
```

One problem of the simulation strategy is that it treats the library size of each cell as an accurate proxy for its total RNA content.
If this is not the case, the simulation will not combine expression profiles in the correct proportion.
...
We can improve the clarity of the result by setting `force.match=TRUE` in the `doubletCells()` call.
...
We can see the effects of doing so in Figure \@ref(fig:densclust2), where more cells in the suspect cluster have higher doublet scores.

```{r densclust2, fig.cap="Distribution of force-matched doublet scores for each cluster in the mammary gland data set. Each point is a cell."}
set.seed(100)
dbl.dens.force <- doubletCells(sce, approximate=TRUE, force.match=TRUE)
summary(dbl.dens.force)
sce$DoubletDensityForced <- dbl.dens.force
plotColData(sce, x="Cluster", y="DoubletDensityForced", colour_by="Cluster")
```

The advantage of `doubletCells()` is that it does not depend on clusters, allowing it to be used more freely throughout a scRNA-seq analysis.
...
Setting `force.match=TRUE` overcomes some of these issues but is a rather aggressive strategy that may promote the spurious identification of doublets.

It should also be stressed that the interpretation of the doublet scores is relative.
It is difficult to define a generalizable threshold above which libraries are to be considered doublets.
Thus, these scores are most often used in the context of cluster annotation, where clusters with higher-than-average doublet scores are considered suspect.
...
To this end, size factors from `computeSumFactors()` (see `r Biocpkg("simpleSingleCell", vignette="xtra-2-spike", label="here")`) can be supplied to `doubletCells()` via `size.factors.content=`.
