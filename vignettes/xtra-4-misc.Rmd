...
date: "`r Sys.Date()`"
vignette: >
  %\VignetteIndexEntry{Specials 4: further strategies for analyzing scRNA-seq data}
...
alternative methods for selecting the number of components from a principal components analysis (PCA),
...
We have previously described the use of the total technical variance to determine the number of principal components (PCs) to retain from a PCA of the scRNA-seq expression matrix.
An alternative strategy for choosing the number of PCs is to use Horn's parallel analysis [@horn1965rationale],
...
To focus on relevant features, we only use genes with positive biological components.
This is identical to the variance modelling used in the [previous workflow](https://bioconductor.org/packages/release/workflows/vignettes/simpleSingleCell/inst/doc/work-1-reads.html#modelling-the-technical-noise-in-gene-expression).

```{r}
sce.416B <- readRDS("416B_data.rds")
fit.416B <- trendVar(sce.416B, parametric=TRUE, block=sce.416B$Plate,
    loess.args=list(span=0.3))
dec.416B <- decomposeVar(sce.416B, fit.416B)
```

We perform parallel analysis using the `parallelPCA()` function from `r Biocpkg("scran")`.
...
npcs <- parallelPCA(sce.416B, assay.type="corrected", 
	subset.row=dec.416B$bio > 0, value="n")
as.integer(npcs)
```
