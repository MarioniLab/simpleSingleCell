...
```{r}
library(edgeR)
summed <- sumTechReps(counts(sce.416b), combos) # need a more efficient method here.
head(summed)
```
...
This strategy yields more accurate $p$-values and fewer false positives than the naive approach.
...
Note that summation only ensures that sample-level variability is modelled correctly.
The data dredging problem mentioned previously is an orthogonal issue that will not be resolved by performing summation on data with empirically defined clusters.
That said, the summation approach can easily be used for _a priori_ defined groups for which data dredging is not a problem.

Summation will alter the ranking of DE genes as it does not explicitly penalize DE genes that are highly variable across cells within each sample.
One might argue that this is problematic as DE genes with low variance across cells are more discriminative and thus more useful as markers.
In practice, this is less of an issue than expected.
...
These effects implicitly favour DE genes with low cell-to-cell variance.
