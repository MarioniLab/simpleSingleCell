...
Our analysis aims to characterize the heterogeneity in transcriptional profiles across different cell types in this dataset.

# Setting up the data 

Public data often requires considerable pre-processing before it is suitable for further analysis.
Fortunately, this work has already been done in the `r Biocpkg("scRNAseq")` package,
which conveniently provides the @zeisel2015brain count data in the form of a `SingleCellExperiment` object.
...
We remove the repeat regions as these are not of interest.
We will use the spike-in transcripts and the mitochondrial proportions later for quality control.
